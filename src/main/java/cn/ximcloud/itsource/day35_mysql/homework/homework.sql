...
SELECT id,productName,dir_id,salePrice FROM product WHERE salePrice>=20 AND productName LIKE '%罗技M1%';
...
-- 查询M系列并按照批发价排序(加上别名)
SELECT *,salePrice*cutoff AS '批发价' FROM product WHERE productName LIKE '%罗技M%' ORDER BY salePrice*cutoff DESC;
-- 查询分类为2并按照批发价排序(加上别名)
